...
export function connect({ ls, es, publicKey, wsUrl }: NoiseConnectOptions) {
  ...
  const rpk = Buffer.from(
    '0339f0d8f98b60539d3781d3237ae34b7cb2967a1ece83803d47229bf4171bc5ad',
    'hex'
  )
  ...
  const socket = new WebSocket('wss://lnsocket.clams.tech/54.253.48.162:7000')
          ...
          .subscribe(async () => {
            ...
            const msg = await noise.encryptMessage(
              Buffer.concat([
                ...
                Buffer.from(
                  JSON.stringify({ method: 'getinfo', rune: request.rune, params: [], id: 253 })
                )
              ])
            )
            socket.send(msg)
